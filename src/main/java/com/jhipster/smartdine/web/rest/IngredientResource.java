...
import com.jhipster.smartdine.repository.IngredientRepository;
...
/**
 * REST controller for managing {@link com.jhipster.smartdine.domain.Ingredient}.
 */
@RestController
...
public class IngredientResource {
    ...
    public ResponseEntity<Ingredient> createIngredient(@Valid @RequestBody Ingredient ingredient) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/ingredients/" + ingredient.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, ingredient.getId().toString()))
            .body(ingredient);
    }
    ...
    public ResponseEntity<Ingredient> partialUpdateIngredient(
        ...
        Optional<Ingredient> result = ingredientRepository
            ...
            .map(existingIngredient -> {
                ...
                if (ingredient.getEnabled() != null) {
                    existingIngredient.setEnabled(ingredient.getEnabled());
                }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<Ingredient>> getAllIngredients(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
